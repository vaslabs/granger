 ...
 */
class SingleStateGrangerRepo()(implicit repo: Repo[Map[PatientId, Patient]], clock: Clock)
  ...
  override def retrieveAllPatients(): IO[Either[RepoErrorState, List[Patient]]] =
    IO {
      repo
        .getState()
        .map(s => {
  ...
  override def addToothInfo(rq: AddToothInformationRequest): Either[InvalidData, IO[Patient]] = {
    val newState = for {
      ...
      tooth <- patient.dentalChart.teeth
        .find(_.number == rq.toothNumber)
        .toRight(ToothNotFound(rq.patientId, rq.toothNumber))
  ...
  override def startTreatment(
    ...
    val newState = for {
      ...
      toothToStartTreatment <- patient.dentalChart.teeth
        .find(_.number == toothId)
        .toRight(ToothNotFound(patientId, toothId))
      toothWithTreatment <- toothToStartTreatment
        .update(treatment)
        .left
        .map(_ => ToothHasActiveTreatment(patientId, toothId))
    ...
    updateStateAndRetrievePatient(s"Started treatment for tooth $toothId on patient $patientId", newState, patientId)
  }
  ...
  def updateStateAndRetrievePatient(
      ...
      patientId: PatientId) = {
    newState.map(_ -> (saveState.map(_ -> patientId).andThen(unsafeGetPatientId))).map {
      case (state, op) =>
        op(msg, state)
    }
  }
  ...
  override def finishTreatment(
      patientId: PatientId,
      toothId: Int,
      finishTime: ZonedDateTime): Either[InvalidData, IO[Patient]] = {

    val newState = for {
      patient <- state.get(patientId).toRight(PatientNotFound(patientId))
      toothWithTreatment <- patient.dentalChart.teeth
        .find(_.number == toothId)
        .toRight(ToothNotFound(patientId, toothId))
      toothWithFinishedTreatment <- toothWithTreatment
        .finishTreatment(finishTime)
    ...
    updateStateAndRetrievePatient(s"Finished treatment for tooth $toothId on patient $patientId", newState, patientId)
  }

  override def loadData(): IO[LoadDataOutcome] =
    retrieveAllPatients().map(_.fold(res => LoadDataFailure(res), _ => LoadDataSuccess))
  ...
  override def deleteTreatment(
    ...
    updateStateAndRetrievePatient(msg, newState, patientId)
  }
  ...
  override def deletePatient(patientId: PatientId): IO[Either[IOError, Unit]] = IO {
    ...
    Either
      .fromOption(newState, PatientNotFound(patientId))
      .flatMap(state => {
