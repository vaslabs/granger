 ...
 */
package object git {
  def getFile(snapshotFile: String, directory: File)(implicit git: Git): Either[RepoErrorState, File] = {
    Either
  ...
  def saveTo[A](snapshotFile: String, location: File, a: A, commitMessage: String)(
      implicit git: Git,
    ...
    fileEither.left.map(e => WriteError("Failure to save data")).flatMap(commitChange(_, commitMessage))
  }
}
