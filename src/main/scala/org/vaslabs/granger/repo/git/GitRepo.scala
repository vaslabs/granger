...
class GitRepo[A](dbLocation: File, snapshotFile: String)(
  ...
  private def setUpRemote(remoteRepo: RemoteRepo): StatusCode =
    ...
    }.map(_ => StatusCodes.Created).getOrElse(StatusCodes.InternalServerError)

  override def push(): Unit = gitApi.push().call()

  override def save(message: String, a: A): Either[IOError, File] =
