...
object RCTReminderActor {
  ...
  private[this] def behaviourWithReminders(
      reminders: Set[Reminder],
      notificationsRepo: GitRepo[List[Reminder]]): Behavior[Protocol] = Behaviors.receiveMessage {
    ...
    case ModifyReminder(timestamp, snoozeTo, externalReference, replyTo: ActorRef[SnoozeAck]) =>
      val savedReminders = for {
        reminderToModify <- reminders.find(
          r =>
        ...
        _ <- notificationsRepo
          .save(s"Saving reminder modification $timestamp of patient id $externalReference", newReminders.toList)
          .toOption
    ...
    case DeleteReminder(timestamp, externalReference, deletionTime, replyTo: ActorRef[DeletedAck]) =>
      val savedReminders = for {
        ...
        _ <- notificationsRepo
          .save(s"Stopping reminder $timestamp of patient id $externalReference", newReminders.toList)
          .toOption
    ...
    case PatientReminders(patientId, replyTo: ActorRef[AllPatientReminders]) =>
      replyTo ! AllPatientReminders(
        reminders
          .filter(_.externalReference == patientId)
          .filter(_.deletedOn.isEmpty)
          .map(r => Notification(r.submitted, r.remindOn, r.externalReference))
          .toList)
      Behaviors.same
  }
}
...
case class SetReminder(
    ...
    actorRef: ActorRef[ReminderSetAck])
    extends Protocol
...
case class DeleteReminder(
    reminderTimestamp: ZonedDateTime,
    externalReference: PatientId,
    deletionTime: ZonedDateTime,
    actorRef: ActorRef[DeletedAck])
    extends Protocol
...
@Lenses private[reminders] case class Reminder(
    ...
    remindOn: ZonedDateTime,
    externalReference: PatientId,
    deletedOn: Option[ZonedDateTime] = None)
    extends Protocol
